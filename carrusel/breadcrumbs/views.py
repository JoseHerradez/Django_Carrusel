...
def generarCarrusel(request, pk=0):
    breadcrumbs = get_object_or_404(BreadcrumbsContent, id=pk)
    elements = breadcrumbs.content_set.all().values()
    data = {
        'title': breadcrumbs.title,
        'elements': elements
    }
    html = armarBreadcrumbsHTML(data)
    js = armarBreadcrumbsJS(data)
    css = armarBreadcrumbsCSS()

    with open('patron/templates/breadcrumbs.html', 'w') as f:
        f.write("{% load static %}\n" + html["file"].render())
    with open('static/patron/script.js', 'w') as f:
        f.write(js)
    return render(request, 'breadcrumbs.html', {'htmlCode': html["code"].render(), 'cssCode': css})
...
def armarBreadcrumbsHTML(data):
    ...
    with doc.head:
        link(href='https://fonts.googleapis.com/css?family=Alegreya+Sans:300,400|Source+Sans+Pro:400,300',
    ...
    with doc:
        with div(cls="slider"):
            ...
            with div(cls="controls"):
                ...
                div(cls="pagination")
            br()
            h3("Para lograr este carrusel en tu pagina web, solo copia el siguiente codigo:",
